import { Category, MenuItem, User, Coupon, InventoryItem, DashboardStats, Order } from '../types';

export const DEFAULT_CATEGORIES: Category[] = [
  { id: 1, name: 'Breakfast', description: 'Start your day right with our nutritious breakfast options', active: true },
  { id: 2, name: 'Lunch', description: 'Hearty meals for your midday break', active: true },
  { id: 3, name: 'Snacks', description: 'Light bites to keep you energized', active: true },
  { id: 4, name: 'Beverages', description: 'Refreshing drinks, hot and cold', active: true },
  { id: 5, name: 'Specials', description: 'Chef specials and seasonal favorites', active: true },
];

export const DEFAULT_MENU_ITEMS: MenuItem[] = [
  {
    id: 1,
    name: 'Masala Dosa',
    description: 'Crispy rice crepe filled with spiced potato masala, served with sambar and chutney',
    price: 80.0,
    categoryId: 1,
    categoryName: 'Breakfast',
    available: true,
    vegetarian: true,
    rating: 4.5,
    ratingCount: 120,
    preparationTime: 10,
    imageUrl: 'https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 2,
    name: 'Idli Sambar',
    description: 'Soft steamed rice cakes served with piping hot sambar and fresh coconut chutney',
    price: 40.0,
    categoryId: 1,
    categoryName: 'Breakfast',
    available: true,
    vegetarian: true,
    rating: 4.3,
    ratingCount: 95,
    preparationTime: 8,
    imageUrl: 'https://images.unsplash.com/photo-1589301760014-d929f3979dbc?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 3,
    name: 'Medu Vada',
    description: 'Crispy lentil doughnuts served with sambar and chutney',
    price: 30.0,
    categoryId: 1,
    categoryName: 'Breakfast',
    available: true,
    vegetarian: true,
    rating: 4.2,
    ratingCount: 80,
    preparationTime: 8,
    imageUrl: 'https://images.unsplash.com/photo-1626777552726-4a6b54c97e46?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 4,
    name: 'Upma',
    description: 'Savory semolina porridge with vegetables and spices',
    price: 45.0,
    categoryId: 1,
    categoryName: 'Breakfast',
    available: true,
    vegetarian: true,
    rating: 4.0,
    ratingCount: 60,
    preparationTime: 10,
    imageUrl: 'https://images.unsplash.com/photo-1645177628172-a94c1f96e6db?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 5,
    name: 'Kesari Bath',
    description: 'Sweet saffron-flavored semolina pudding with ghee',
    price: 35.0,
    categoryId: 1,
    categoryName: 'Breakfast',
    available: true,
    vegetarian: true,
    rating: 4.4,
    ratingCount: 75,
    preparationTime: 8,
    imageUrl: '/images/kesari-bath.png',
  },
  {
    id: 6,
    name: 'Pulao',
    description: 'Fragrant basmati rice cooked with mixed vegetables and aromatic spices',
    price: 90.0,
    categoryId: 2,
    categoryName: 'Lunch',
    available: true,
    vegetarian: true,
    rating: 4.1,
    ratingCount: 55,
    preparationTime: 15,
    imageUrl: 'https://images.unsplash.com/photo-1633945274405-b6c8069047b0?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 7,
    name: 'Dal Tadka',
    description: 'Yellow lentils tempered with cumin, garlic and spices, served with rice',
    price: 75.0,
    categoryId: 2,
    categoryName: 'Lunch',
    available: true,
    vegetarian: true,
    rating: 4.3,
    ratingCount: 65,
    preparationTime: 15,
    imageUrl: 'https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 8,
    name: 'Paneer Butter Masala',
    description: 'Cottage cheese in rich tomato-butter gravy, best with naan',
    price: 130.0,
    categoryId: 2,
    categoryName: 'Lunch',
    available: true,
    vegetarian: true,
    rating: 4.7,
    ratingCount: 110,
    preparationTime: 20,
    imageUrl: 'https://images.unsplash.com/photo-1631452180519-c014fe946bc7?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 9,
    name: 'Chicken Biryani',
    description: 'Aromatic basmati rice cooked with tender chicken and whole spices',
    price: 160.0,
    categoryId: 2,
    categoryName: 'Lunch',
    available: true,
    vegetarian: false,
    rating: 4.8,
    ratingCount: 200,
    preparationTime: 25,
    imageUrl: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 10,
    name: 'Egg Fried Rice',
    description: 'Wok-tossed rice with scrambled eggs, vegetables and soy sauce',
    price: 100.0,
    categoryId: 2,
    categoryName: 'Lunch',
    available: true,
    vegetarian: false,
    rating: 4.2,
    ratingCount: 90,
    preparationTime: 15,
    imageUrl: 'https://images.unsplash.com/photo-1603133872878-684f208fb84b?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 11,
    name: 'Samosa',
    description: 'Crispy pastry filled with spiced potatoes and peas, served with mint chutney',
    price: 20.0,
    categoryId: 3,
    categoryName: 'Snacks',
    available: true,
    vegetarian: true,
    rating: 4.5,
    ratingCount: 180,
    preparationTime: 5,
    imageUrl: 'https://images.unsplash.com/photo-1601050690597-df0568f70950?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 12,
    name: 'Bread Pakoda',
    description: 'Bread slices dipped in spiced chickpea batter and deep fried',
    price: 25.0,
    categoryId: 3,
    categoryName: 'Snacks',
    available: true,
    vegetarian: true,
    rating: 4.1,
    ratingCount: 70,
    preparationTime: 5,
    imageUrl: 'https://images.unsplash.com/photo-1606491956689-2ea866880c84?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 13,
    name: 'Poha',
    description: 'Flattened rice cooked with onions, mustard seeds, and curry leaves',
    price: 30.0,
    categoryId: 3,
    categoryName: 'Snacks',
    available: true,
    vegetarian: true,
    rating: 4.0,
    ratingCount: 55,
    preparationTime: 8,
    imageUrl: 'https://images.unsplash.com/photo-1610057099443-fde8c4d50f91?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 14,
    name: 'Masala Chai',
    description: 'Traditional Indian spiced tea with ginger, cardamom and milk',
    price: 20.0,
    categoryId: 4,
    categoryName: 'Beverages',
    available: true,
    vegetarian: true,
    rating: 4.6,
    ratingCount: 250,
    preparationTime: 3,
    imageUrl: 'https://images.unsplash.com/photo-1576092768241-dec231879fc3?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 15,
    name: 'Cold Coffee',
    description: 'Blended iced coffee with milk and sugar',
    price: 60.0,
    categoryId: 4,
    categoryName: 'Beverages',
    available: true,
    vegetarian: true,
    rating: 4.4,
    ratingCount: 130,
    preparationTime: 5,
    imageUrl: 'https://images.unsplash.com/photo-1517701550927-30cf4ba1dba5?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 16,
    name: 'Fresh Lime Soda',
    description: 'Freshly squeezed lime with soda water, sweet or salted',
    price: 30.0,
    categoryId: 4,
    categoryName: 'Beverages',
    available: true,
    vegetarian: true,
    rating: 4.3,
    ratingCount: 95,
    preparationTime: 3,
    imageUrl: 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 17,
    name: 'Mango Lassi',
    description: 'Chilled yogurt drink blended with fresh mango pulp',
    price: 55.0,
    categoryId: 4,
    categoryName: 'Beverages',
    available: true,
    vegetarian: true,
    rating: 4.7,
    ratingCount: 160,
    preparationTime: 5,
    imageUrl: '/images/mango-lassi.jpg',
  },
  {
    id: 18,
    name: 'Thali Special',
    description: 'Complete meal with dal, sabzi, rice, roti, salad and dessert',
    price: 150.0,
    categoryId: 5,
    categoryName: 'Specials',
    available: true,
    vegetarian: true,
    rating: 4.9,
    ratingCount: 85,
    preparationTime: 20,
    imageUrl: 'https://images.unsplash.com/photo-1589302168068-964664d93dc0?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 19,
    name: 'Chef Special Pasta',
    description: 'Penne pasta in homemade tomato basil sauce with herbs',
    price: 120.0,
    categoryId: 5,
    categoryName: 'Specials',
    available: true,
    vegetarian: true,
    rating: 4.2,
    ratingCount: 45,
    preparationTime: 20,
    imageUrl: 'https://images.unsplash.com/photo-1551183053-bf91a1d81141?auto=format&fit=crop&w=600&q=80',
  },
  {
    id: 20,
    name: 'Chaat Platter',
    description: 'Assorted street food bites: pani puri, bhel puri and sev puri',
    price: 70.0,
    categoryId: 5,
    categoryName: 'Specials',
    available: true,
    vegetarian: true,
    rating: 4.6,
    ratingCount: 100,
    preparationTime: 10,
    imageUrl: '/images/chaat-platter.png',
  },
];

export const DEFAULT_DASHBOARD_STATS: DashboardStats = {
  totalOrders: 248,
  todayOrders: 42,
  totalRevenue: 28450.0,
  todayRevenue: 4680.0,
  totalCustomers: 320,
  availableMenuItems: 20,
  pendingOrders: 5,
  lowStockCount: 2,
  ordersByStatus: {
    PLACED: 3,
    CONFIRMED: 2,
    PREPARING: 4,
    READY_FOR_PICKUP: 3,
    COMPLETED: 228,
    CANCELLED: 8,
  },
  topSellingItems: [
    { name: 'Masala Dosa', quantitySold: 120, revenue: 9600 },
    { name: 'Chicken Biryani', quantitySold: 98, revenue: 15680 },
    { name: 'Cold Coffee', quantitySold: 85, revenue: 5100 },
    { name: 'Samosa', quantitySold: 140, revenue: 2800 },
    { name: 'Thali Special', quantitySold: 65, revenue: 9750 },
  ],
};

export const DEFAULT_ADMIN_ORDERS: Order[] = [
  {
    id: 101,
    orderNumber: 'ORD-2026-9482',
    userId: 3,
    userName: 'Rahul Sharma',
    userEmail: 'rahul@example.com',
    userPhone: '9876543210',
    status: 'PREPARING',
    totalAmount: 140.0,
    discountAmount: 14.0,
    taxAmount: 6.3,
    finalAmount: 132.3,
    couponCode: 'WELCOME10',
    paymentMethod: 'UPI',
    paymentStatus: 'COMPLETED',
    notes: 'Extra chutney please',
    items: [
      {
        id: 1,
        menuItemId: 1,
        itemName: 'Masala Dosa',
        quantity: 1,
        unitPrice: 80.0,
        totalPrice: 80.0,
        imageUrl: 'https://images.unsplash.com/photo-1668236543090-82eba5ee5976?auto=format&fit=crop&w=600&q=80',
      },
      {
        id: 2,
        menuItemId: 15,
        itemName: 'Cold Coffee',
        quantity: 1,
        unitPrice: 60.0,
        totalPrice: 60.0,
        imageUrl: 'https://images.unsplash.com/photo-1517701550927-30cf4ba1dba5?auto=format&fit=crop&w=600&q=80',
      },
    ],
    createdAt: new Date(Date.now() - 15 * 60 * 1000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 102,
    orderNumber: 'ORD-2026-8831',
    userId: 4,
    userName: 'Priya Patel',
    userEmail: 'priya@example.com',
    userPhone: '9765432109',
    status: 'READY_FOR_PICKUP',
    totalAmount: 160.0,
    discountAmount: 0.0,
    taxAmount: 8.0,
    finalAmount: 168.0,
    paymentMethod: 'CARD',
    paymentStatus: 'COMPLETED',
    items: [
      {
        id: 3,
        menuItemId: 9,
        itemName: 'Chicken Biryani',
        quantity: 1,
        unitPrice: 160.0,
        totalPrice: 160.0,
        imageUrl: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=600&q=80',
      },
    ],
    createdAt: new Date(Date.now() - 30 * 60 * 1000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 103,
    orderNumber: 'ORD-2026-7724',
    userId: 5,
    userName: 'Amit Kumar',
    userEmail: 'amit@example.com',
    userPhone: '9654321098',
    status: 'CONFIRMED',
    totalAmount: 175.0,
    discountAmount: 20.0,
    taxAmount: 7.75,
    finalAmount: 162.75,
    couponCode: 'SAVE20',
    paymentMethod: 'UPI',
    paymentStatus: 'COMPLETED',
    items: [
      {
        id: 4,
        menuItemId: 8,
        itemName: 'Paneer Butter Masala',
        quantity: 1,
        unitPrice: 130.0,
        totalPrice: 130.0,
        imageUrl: 'https://images.unsplash.com/photo-1631452180519-c014fe946bc7?auto=format&fit=crop&w=600&q=80',
      },
      {
        id: 5,
        menuItemId: 4,
        itemName: 'Upma',
        quantity: 1,
        unitPrice: 45.0,
        totalPrice: 45.0,
        imageUrl: 'https://images.unsplash.com/photo-1645177628172-a94c1f96e6db?auto=format&fit=crop&w=600&q=80',
      },
    ],
    createdAt: new Date(Date.now() - 45 * 60 * 1000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 104,
    orderNumber: 'ORD-2026-6612',
    userId: 3,
    userName: 'Rahul Sharma',
    userEmail: 'rahul@example.com',
    status: 'PLACED',
    totalAmount: 75.0,
    discountAmount: 0.0,
    taxAmount: 3.75,
    finalAmount: 78.75,
    paymentMethod: 'CASH_ON_PICKUP',
    paymentStatus: 'PENDING',
    items: [
      {
        id: 6,
        menuItemId: 7,
        itemName: 'Dal Tadka',
        quantity: 1,
        unitPrice: 75.0,
        totalPrice: 75.0,
        imageUrl: 'https://images.unsplash.com/photo-1546833999-b9f581a1996d?auto=format&fit=crop&w=600&q=80',
      },
    ],
    createdAt: new Date(Date.now() - 5 * 60 * 1000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 105,
    orderNumber: 'ORD-2026-5501',
    userId: 4,
    userName: 'Priya Patel',
    userEmail: 'priya@example.com',
    status: 'COMPLETED',
    totalAmount: 150.0,
    discountAmount: 0.0,
    taxAmount: 7.5,
    finalAmount: 157.5,
    paymentMethod: 'UPI',
    paymentStatus: 'COMPLETED',
    items: [
      {
        id: 7,
        menuItemId: 18,
        itemName: 'Thali Special',
        quantity: 1,
        unitPrice: 150.0,
        totalPrice: 150.0,
        imageUrl: 'https://images.unsplash.com/photo-1589302168068-964664d93dc0?auto=format&fit=crop&w=600&q=80',
      },
    ],
    createdAt: new Date(Date.now() - 120 * 60 * 1000).toISOString(),
    updatedAt: new Date().toISOString(),
  },
];

export const DEFAULT_INVENTORY: InventoryItem[] = DEFAULT_MENU_ITEMS.map((item) => ({
  id: item.id,
  menuItemId: item.id,
  itemName: item.name,
  categoryName: item.categoryName,
  quantity: item.id === 9 ? 8 : item.id === 8 ? 6 : 45 + ((item.id * 7) % 50),
  unit: item.id === 14 ? 'cups' : item.id === 15 || item.id === 16 || item.id === 17 ? 'glasses' : item.id === 11 || item.id === 12 ? 'pieces' : 'plates',
  minStockLevel: 10,
  lowStock: item.id === 9 || item.id === 8,
  lastUpdated: new Date().toISOString(),
}));

export const DEFAULT_USERS: User[] = [
  {
    id: 1,
    name: 'Admin User',
    email: 'admin@canteen.com',
    phone: '9999999999',
    roles: ['ADMIN', 'CUSTOMER'],
    enabled: true,
    createdAt: '2026-01-10T10:00:00Z',
  },
  {
    id: 2,
    name: 'Kitchen Staff',
    email: 'staff@canteen.com',
    phone: '9888888888',
    roles: ['STAFF'],
    enabled: true,
    createdAt: '2026-01-15T11:00:00Z',
  },
  {
    id: 3,
    name: 'Rahul Sharma',
    email: 'rahul@example.com',
    phone: '9876543210',
    roles: ['CUSTOMER'],
    enabled: true,
    createdAt: '2026-02-01T09:30:00Z',
  },
  {
    id: 4,
    name: 'Priya Patel',
    email: 'priya@example.com',
    phone: '9765432109',
    roles: ['CUSTOMER'],
    enabled: true,
    createdAt: '2026-02-14T14:15:00Z',
  },
  {
    id: 5,
    name: 'Amit Kumar',
    email: 'amit@example.com',
    phone: '9654321098',
    roles: ['CUSTOMER'],
    enabled: true,
    createdAt: '2026-03-01T16:45:00Z',
  },
];

export const DEFAULT_COUPONS: Coupon[] = [
  {
    id: 1,
    code: 'WELCOME10',
    description: 'Welcome discount for new student accounts',
    discountPercent: 10,
    maxDiscount: 50,
    minOrderAmount: 100,
    expiryDate: '2027-12-31',
    usageLimit: 1000,
    usedCount: 142,
    active: true,
  },
  {
    id: 2,
    code: 'SAVE20',
    description: '20% off on all campus orders above Rs.200',
    discountPercent: 20,
    maxDiscount: 80,
    minOrderAmount: 200,
    expiryDate: '2027-06-30',
    usageLimit: 500,
    usedCount: 89,
    active: true,
  },
  {
    id: 3,
    code: 'LUNCH15',
    description: '15% discount for midday canteen lunch orders',
    discountPercent: 15,
    maxDiscount: 60,
    minOrderAmount: 150,
    expiryDate: '2027-12-31',
    usageLimit: 800,
    usedCount: 215,
    active: true,
  },
  {
    id: 4,
    code: 'SPECIAL50',
    description: 'Flat 50% discount festival flash deal',
    discountPercent: 50,
    maxDiscount: 100,
    minOrderAmount: 300,
    expiryDate: '2026-12-31',
    usageLimit: 100,
    usedCount: 76,
    active: true,
  },
];
